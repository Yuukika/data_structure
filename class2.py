...
import time
"""
线性表：具有相同特性的数据元素额一个有限序列
基本运算集包括：
初始化，删除表，是否为空，求长度，输出线性表，查找数据，插入数据，删除数据
存储方式包含两种：顺序表和链表
    顺序表中的数据是紧挨着的，即在物理地址上是前后的关系，通过数组来实现

    链表，每个存储节点不仅包含着元素本身的信息，而且包含元素之间的逻辑关系信息，
前驱节点中包含着后继节点的地址信息，初始化时为None,即不需要指向其他任何节点。
    在每个节点中除包含有数据域以外，只设置一个指针域，用以指向后继节点，这样的链表称为单链表
    如果设置两个指针域，分别用于指向前驱节点和后继节点，这样称为双链表
